...
- **`--vllm-endpoint`** (one or more strings, default: `"http://localhost:8000/v1"`) vLLM endpoint addresses for generating hidden states on-demand (online training). Pass multiple URLs after one option, or repeat the option. Requests are distributed round-robin with rank/worker-specific starting offsets. Every service's returned hidden-states path must be accessible from the training machine. Ignored if `--on-missing` is not set to `generate`.

  ```bash
  --vllm-endpoint http://10.0.0.11:8000/v1 http://10.0.0.12:8000/v1
  ```
